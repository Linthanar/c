...
int main(int argc, char *argv[]) {
    ...
    for(w=0;w < ILOSC_WIERSZY; w++){
        for(k = 0;tab[w][k]!='\n';k++){
            if(tab[w+1][k+1] == '$'){
                tab[w+0][k] = '^'; tab[w+0][k+1] = '^';if(tab[w+0][k+2] != '\n'){tab[w+0][k+2] = '^';}
                tab[w+1][k] = '^'; tab[w+1][k+1] = '^';if(tab[w+0][k+2] != '\n'){tab[w+1][k+2] = '^';}
                tab[w+2][k] = '^'; tab[w+2][k+1] = '^';if(tab[w+0][k+2] != '\n'){tab[w+2][k+2] = '^';}
            }
        }    
    }
    /*otworz plik z tablicy 2[plik] -> jeśli tablica pusta to zakończ "return 1"*/    
    FILE *plik_w = fopen(argv[2], "w");
    if (plik_w == NULL) { return 1; }
    /*pętle poniżej wypisują do pliku znak po znaku i gdy napotkają '\n' to przerywa zapisuje nową linię
    tyle razy ile jest wierszy*/
    w = 0, k = 0;
    for(w=0;w < ILOSC_WIERSZY; w++){
        for(k = 0;tab[w][k]!='\n';k++){
            fprintf(plik_w,"%c ",tab[w][k]);
        }
        fprintf(plik_w,"\n");
    }
